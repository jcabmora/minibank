...
docker run -d --name mysql -e MYSQL_ROOT_PASSWORD=hobbes -v `pwd`/scripts:/docker-entrypoint-initdb.d:ro -d --network minibanknet gcr.io/`gcloud config get-value project`/mariadb:latest
